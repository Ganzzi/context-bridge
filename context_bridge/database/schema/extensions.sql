...
CREATE TABLE IF NOT EXISTS chunks (
    ...
    bm25_vector bm25_catalog.bm25vector, -- Auto-generated by trigger
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE (
        document_id,
        group_id,
...
CREATE INDEX IF NOT EXISTS idx_chunks_bm25 ON chunks USING bm25 (bm25_vector bm25_catalog.bm25_ops);

-- Trigger: Auto-generate bm25_vector from content
CREATE OR REPLACE FUNCTION generate_bm25_vector()
RETURNS TRIGGER AS $$
BEGIN
    NEW.bm25_vector := tokenizer_catalog.tokenize(NEW.content, 'bert');
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- Create bert tokenizer for BM25 using llmlingua2 model
DO $$
BEGIN
    PERFORM tokenizer_catalog.create_tokenizer('bert', 'model = "llmlingua2"');
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Tokenizer creation may have failed: %', SQLERRM;
END
$$;
